        ...
        alertDialog.setPositiveButton("tamam", (dialog, which) -> {
            ...
            mpasscode.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    String passc = Objects.requireNonNull(snapshot.child("sifre").getValue()).toString().trim();
                    if (passc.equals(sifre)){
                        progressDialog.show();
                        auth.createUserWithEmailAndPassword(etEmail.getText().toString().trim(), etPassword.getText().toString().trim()).
                                addOnCompleteListener(task1 -> {
                                    ...
                                    if (task1.isSuccessful()) {
                                        ...
                                        Toast.makeText(SignUpActivity.this, "hata:" + task1.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                        finish();
                                    }
                                });
                    }else {
